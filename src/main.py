...
def main():
    ...
    generate_page_recursive("content", "template.html", public_path)
...
# Path, Template, Path
# Takes all MD from from_path and it's sub folders, 
# html template from template_path, and copies new 
# HTML doc to dest_path
def generate_page_recursive(from_path, template_path, dest_path):
    for item in os.listdir(from_path):
        path = os.path.join(from_path, item)
        if os.path.isfile(path):
            print(f"Generating page from {path} to {dest_path} using {template_path}")
            ...
            print (f"{path} is a subfolder, moved to {new_dest}")
            os.mkdir(new_dest)
